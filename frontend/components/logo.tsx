...
export function Logo({ compact = false }: LogoProps) {
  return (
    <Link href="/" className="group inline-flex items-center gap-3">
      <span className="relative inline-flex h-11 w-11 items-center justify-center overflow-hidden rounded-2xl border border-white/10 bg-white/5 shadow-card">
        ...
        <svg
          viewBox="0 0 64 64"
          className="relative h-7 w-7 text-white transition duration-300 group-hover:scale-105"
          fill="none"
      ...
      {!compact ? (
        <span className="flex flex-col">
          ...
          <span className="text-xl font-black tracking-[0.22em] text-white">FLIX</span>
        </span>
      ) : null}
    </Link>
  );
